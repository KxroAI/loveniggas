...
class AdminCog(commands.Cog):
    ...
    @app_commands.command(name="nuke", description="Clone this channel to wipe all messages (requires Administrator)")
    @app_commands.describe(channel="Channel to nuke (defaults to current channel)")
    async def nuke(self, interaction: discord.Interaction, channel: discord.TextChannel = None):
        if interaction.user.id != BOT_OWNER_ID and not interaction.user.guild_permissions.administrator:
            return await interaction.response.send_message(
                embed=create_embed(description="❌ You need **Administrator** permission to nuke a channel."),
                ephemeral=True,
            )
        ...
        confirm_embed = discord.Embed(
            description=f"⚠️ Are you sure you want to nuke {channel.mention}?\n**This will delete ALL messages** by cloning the channel.",
            color=discord.Color.red(),
        )
        view = discord.ui.View(timeout=30)
        confirmed = False

        yes_btn = discord.ui.Button(label="Yes, Nuke It", style=discord.ButtonStyle.danger, emoji="💣")
        ...
        async def yes_callback(btn_inter: discord.Interaction):
            ...
            if btn_inter.user.id != interaction.user.id:
                return await btn_inter.response.send_message("❌ Not your command.", ephemeral=True)
            confirmed = True
        ...
        async def no_callback(btn_inter: discord.Interaction):
            ...
            await btn_inter.response.edit_message(
                embed=discord.Embed(description="✅ Nuke cancelled.", color=discord.Color.green()), view=None
            )
            view.stop()
        ...
        try:
            ...
            await new_channel.edit(position=position, topic=topic, slowmode_delay=slowmode, nsfw=nsfw)
            ...
            nuke_embed = create_embed(
                title="💣 Channel Nuked",
                description=f"This channel was nuked by {interaction.user.mention}.",
            )
            await new_channel.send(embed=nuke_embed)
        except discord.Forbidden:
            await interaction.followup.send("❌ I don't have permission to clone/delete that channel.", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"❌ Error: {e}", ephemeral=True)
