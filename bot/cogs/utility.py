...
class UtilityCog(commands.Cog):
    ...
    @app_commands.command(name="membercount", description="Show a breakdown of this server's member count")
    async def membercount(self, interaction: discord.Interaction):
        ...
        embed = create_embed(title=f"👥 Member Count — {guild.name}")
    ...
    @app_commands.command(name="boostcount", description="Show server boost count and Nitro level")
    async def boostcount(self, interaction: discord.Interaction):
        guild   = interaction.guild
        boosts  = guild.premium_subscription_count or 0
        tier    = guild.premium_tier
        tiers   = {0: (2, "No perks"), 1: (15, "Level 1"), 2: (30, "Level 2"), 3: (None, "Level 3 (Max)")}
        _, tier_name = tiers.get(tier, (None, f"Level {tier}"))
        next_threshold = tiers.get(tier + 1, (None, None))[0] if tier < 3 else None

        embed = create_embed(title=f"🚀 Server Boosts — {guild.name}")
        embed.add_field(name="🔥 Total Boosts", value=f"`{boosts}`", inline=True)
        embed.add_field(name="⭐ Boost Level", value=f"`{tier_name}`", inline=True)
        if next_threshold:
            needed = next_threshold - boosts
            embed.add_field(
                name="📈 Next Level",
                value=f"`{needed} more boost{'s' if needed != 1 else ''} needed`",
                inline=True,
            )
        if guild.icon:
            embed.set_thumbnail(url=guild.icon.url)
    ...
    @app_commands.describe(role="The role to get info on")
    async def roleinfo(self, interaction: discord.Interaction, role: discord.Role):
        ...
        if role.permissions.administrator:
            ...
            for attr, label in perm_map.items():
                if getattr(role.permissions, attr, False):
                    key_perms.append(label)

        embed = create_embed(title=f"🎭 Role Info — {role.name}")
        embed.colour = role.colour if role.colour.value else discord.Colour.blurple()
        embed.add_field(name="🆔 ID", value=f"`{role.id}`", inline=True)
        embed.add_field(name="👥 Members", value=f"`{member_count:,}`", inline=True)
        embed.add_field(name="🎨 Color", value=f"`{str(role.colour)}`", inline=True)
